...
# Read in CV .xlsx
map_df(CV, ~ here(paste0('DOCUMENTATION/DEMO-INPUT-TABLES/CV', .x, '.xlsx')) %>% 
  excel_sheets() %>%
  set_names() %>%
  map_df(read_excel,
         path = here(paste0('DOCUMENTATION/DEMO-INPUT-TABLES/CV', .x, '.xlsx')),
         .id = 'form') %>% 
  assign(paste0('CV_', .x, '_lookup'), ., envir = .GlobalEnv)
)
